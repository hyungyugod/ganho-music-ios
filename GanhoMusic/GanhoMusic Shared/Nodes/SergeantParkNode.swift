...
/// 모든 좌표/크기는 SKSpriteNode 중심(0,0) 기준, UILayout 상수만 사용 — 매직 넘버 0.
final class SergeantParkNode: SKSpriteNode {
    ...
    init() {
        ...
        let visualSize = CGSize(
            width:  UILayout.sergeantParkWidth  * GameplayTuning.pixelSpriteScale,
            height: UILayout.sergeantParkHeight * GameplayTuning.pixelSpriteScale
        )
    ...
    private func attachShadow() {
        let shadow = SKShapeNode(ellipseOf: UILayout.sergeantShadowSize)
        shadow.fillColor = UIColor.black.withAlphaComponent(0.18)
        shadow.strokeColor = .clear
        shadow.position = CGPoint(x: 0, y: UILayout.sergeantShadowOffsetY)
    ...
    private func attachBody() {
        let body = SKShapeNode(rectOf: UILayout.sergeantBodySize, cornerRadius: 1.5)
        body.fillColor = .ganhoAirforceTeal
        body.strokeColor = .ganhoAirforceTealLight
        body.lineWidth = 0.6
        body.position = CGPoint(x: 0, y: UILayout.sergeantBodyOffsetY)
        body.zPosition = 0.1
        addChild(body)
    }
    ...
    private func attachHead() {
        let head = SKShapeNode(circleOfRadius: UILayout.sergeantHeadRadius)
        head.fillColor = .ganhoSkinTone
        head.strokeColor = .ganhoCoralShadow
        head.lineWidth = 0.4
        head.position = CGPoint(x: 0, y: UILayout.sergeantHeadOffsetY)
        head.zPosition = 0.2
        addChild(head)
    }
    ...
    private func attachCap() {
        let crown = SKShapeNode(rectOf: UILayout.sergeantCapCrownSize, cornerRadius: 1.5)
        crown.fillColor = .ganhoAirforceTeal
        crown.strokeColor = .ganhoAirforceTealLight
        crown.lineWidth = 0.5
        crown.position = CGPoint(x: 0, y: UILayout.sergeantCapCrownOffsetY)
        crown.zPosition = 0.3
        addChild(crown)

        let visor = SKShapeNode(rectOf: UILayout.sergeantCapVisorSize)
        visor.fillColor = .ganhoSunglassesBlack
        visor.strokeColor = .clear
        visor.position = CGPoint(x: 0, y: UILayout.sergeantCapVisorOffsetY)
    ...
    private func attachSunglasses() {
        let glasses = SKShapeNode(
            rectOf: UILayout.sergeantSunglassesSize,
            cornerRadius: 0.6
        )
        glasses.fillColor = .ganhoSunglassesBlack
        glasses.strokeColor = .ganhoNavyDeep
        glasses.lineWidth = 0.4
        glasses.position = CGPoint(x: 0, y: UILayout.sergeantSunglassesOffsetY)
        glasses.zPosition = 0.4
        addChild(glasses)
    }

    /// 우측 어깨 v자 계급장 2개 — 골드 stroke chevron. 병장(2 chevron) 의미.
    /// for-loop로 chevron 개수만큼 부착 → UILayout.sergeantRankChevronCount 변경 시 자동 반영.
    private func attachRank() {
        for index in 0..<UILayout.sergeantRankChevronCount {
            let chevron = makeChevronNode()
            chevron.position = CGPoint(
                x: UILayout.sergeantRankOffsetX,
                y: UILayout.sergeantRankOffsetY
                    + CGFloat(index) * UILayout.sergeantRankChevronGap
            )
    ...
    private func makeChevronNode() -> SKShapeNode {
        ...
        let halfWidth  = UILayout.sergeantChevronWidth / 2
        let depth      = UILayout.sergeantChevronHeight
        ...
        shape.lineWidth   = UILayout.sergeantChevronLineWidth
