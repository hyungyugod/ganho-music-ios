...
//  ProjectileNode(F)와 분리된 별도 PhysicsCategory.stethoscope 사용.
...
///
/// Spring 비유: ProjectileNode가 일반 비즈니스 이벤트라면, StethoscopeNode는 *특수 캠페인 이벤트* —
...
final class StethoscopeNode: SKSpriteNode {
    ...
    init() {
        ...
        body.collisionBitMask    = 0       // 통과(벽에 막혀 멈추는 버그 회피, ProjectileNode 패턴 답습)
