...
> (REFACTOR_STATE의 "GANHO_BOOT_SCENE 9종" 기록은 구식 — 실측 **15종**, 본 문서가 진실.)

## 1. 환경변수 9종 + GANHO_BOOT_SCENE

| 환경변수 | 읽는 위치 | 효과 |
|---|---|---|
| `GANHO_BOOT_SCENE={값}` | `GameViewController.debugBootScene(named:)` | 아래 15값 씬 직행 (스크린샷·QA 자동화). 미해당 문자열은 StartScene 폴백 |
...
| `GANHO_FORCE_CONTROLS_HINT=1` (R9 신설) | `GameScene+Countdown.swift` 85행 (`#if DEBUG` 83-88 격리) | 첫 판 조작 온보딩 힌트(ControlsHintNode)를 UserDefaults 표시 이력 무시하고 강제 표시 — 스크린샷 게이트 재현용 (GANHO_SKIP_CUTSCENE 전례 동형) |
| `GANHO_SETTINGS_CREDITS=1` (R9 신설) | `StartScene+Settings.swift` 66행 (`#if DEBUG` 64-69 격리) | 설정 다이얼로그 제시 직후 크레딧 모드 직행 — **`GANHO_BOOT_SCENE=startSettings` 조합 필요** |
...
## 2. GANHO_BOOT_SCENE 15값 (GameViewController.debugBootScene)
...
| `startSettings` | StartScene | shouldOpenSettingsOnEntry=true | 설정 다이얼로그 자동 오픈 (GameViewController.swift:132-134) |
