...
## 디자인 리뉴얼 모드 — Sprint 10 Phase 모드 (확장 / 원본 1:1 픽셀 이식)

Sprint 10는 **원본 웹게임(hyungyugod.github.io/pages/game.html)과 인게임을 시각·게임플레이·수치 1:1 일치화**하는 대규모 리팩토링이다. 메뉴/선택창은 보존, 인게임만 픽셀 게임으로 전면 재구성. 트리거 `Sprint 10 진행해줘`가 들어오면 일반 디자인 리뉴얼 모드 위에 다음 절차가 추가된다.

### Phase 자동 실행 절차

**1. SPRINT_10_REQUEST.md 읽기 (필수 첫 단계)**

- §1 Phase 구조 표에서 10개 Phase 목록 확인
- `DESIGN_RENEWAL_STATE.md` Sprint 10 행에서 합격/대기 Phase 확인 (없으면 Phase A부터)
- 단일 진실 원천 3종(`SPRINT_10_REQUEST.md`, `docs/ORIGINAL_GAME_ANALYSIS.md`, `docs/IOS_CURRENT_STATE.md`) 함께 검토

**2. 다음 Phase 결정**

- 직전 Phase가 "✅ 합격" → 다음 Phase
- 직전 Phase가 "❌ 불합격" → 같은 Phase 재실행 (최대 3회)
- 사용자가 `Sprint 10 Phase X 진행해줘` 명시 → 해당 Phase 강제 실행
...
단계 1: Planner 프롬프트에 다음을 추가:
```
SPRINT_10_REQUEST.md를 읽어라. Sprint 10 작업지시서의 §[해당 Phase 섹션]을 그대로 따를 것.
원본 좌표·수치·AI는 docs/ORIGINAL_GAME_ANALYSIS.md(1391줄)에서만 가져온다 — 라인 번호 명시 필수.
iOS 현재 상태는 docs/IOS_CURRENT_STATE.md를 참고.
사용자 의사결정 7건은 SPRINT_10_REQUEST.md §3을 SPEC.md에 그대로 복사할 것.
변경 금지 항목과 합격 기준을 SPEC.md에 그대로 복사하라.

현재 Phase: [A~J 중 하나]
범위: SPRINT_10_REQUEST.md §4 Phase [X] 그대로
```

단계 2~3: Generator → Evaluator (기본 하네스 동일)

단계 4: 판정. 5축 가중 평균 7.5 이상 → 합격.

**4. Phase 합격 처리 — 상태 갱신**

- 가중 평균 7.5 이상 → DESIGN_RENEWAL_STATE.md "Sprint 10 — 진행 로그"에 `- Phase X ✅ 합격 (점수 X.X/10, 시도 X회, YYYY-MM-DD)` 한 줄 추가
- 다음 Phase 자동 진행 (Phase J까지 완료 시 Sprint 10 전체 합격 처리)

**5. 사용자 보고 형식 (Phase 단위)**

```
## Sprint 10 Phase X 완료
...
- 원본 1:1 일치도: [상세]
- QA 반복: X회
- 최종 점수: 패턴 X.X / 1:1 X.X / 성능 X.X / 시각 X.X / 기능 X.X (가중 X.X/10)
- 다음 Phase: [X+1] 또는 "🎉 Sprint 10 전체 완료"
```

### Phase 의존성

A → B → C/D 병렬 → E → F → G → H → I → J 권장. Phase A(픽셀 시각 노출)는 모든 후속 Phase의 전제이므로 가장 먼저. Phase I(수치 정합)는 C/D/E/F의 거동이 확정된 후에야 의미 있음. Phase J(HUD/이펙트 픽셀화)는 마지막. 사용자가 명시적으로 다른 순서를 요청하면 그것을 따른다.

### 사용자 의사결정 (사전 확정 — 2026-05-21)

Sprint 10 Planner는 SPRINT_10_REQUEST.md §3의 7개 결정 사항을 SPEC.md에 그대로 복사 — 요약:

1. **카메라**: follow 유지 (맵 자체는 32×20 원본 동일)
2. **박병장**: SergeantParkNode 유지 + 원본 비행기/폭탄/도주 5초 흐름 결합
3. **EnemyNode 자식 시각**: 성적표/Hello PPT/클립 모두 제거 — 16×20 픽셀만
4. **iOS 고유 이펙트**: 픽셀 톤으로 변환해서 유지 (콤보 팝업/sparkle/마일스톤/긴박감)
5. **BGM/SE**: 유지
6. **dpad 조작**: 유지 (원본 키보드 ↔ iOS dpad 매핑)
7. **캐릭터 5명 매핑**: 원본 jung/geon/im/lee/kim ↔ iOS 5명 캐릭터 ID 정렬 (Phase A에서 매핑표 확정)

### 비전제 (Sprint 10 동안 절대 건드리지 말 것)

- 메뉴/선택창 6개 씬 전부
- 메뉴 전용 노드 (CharacterFaceNode, CharacterCardNode, DifficultyCardNode 등)
- `PixelSprite.swift` / `PixelPalette.swift` / `PixelSpriteRenderer.swift` 본체 (이미 원본 byte-equal)
- SVG path 캐릭터를 인게임에서 다시 사용하지 말 것 — 인게임은 픽셀만

---

## 그랜드 리팩토링 모드 — v3 "Night Shift" (최신, 2026-06-10)

사용자가 다음 트리거 중 하나를 말하면 **그랜드 리팩토링 모드** 진입:
- "리팩토링 진행해줘"
- "Sprint R 진행해줘" / "R[N] 진행해줘" / "리팩토링 R[N] 진행해줘"

전체 게임을 프리미엄 픽셀 톤으로 통일하고 엔진·게임필·메타 시스템·코드 구조를 재구축하는 9단계(R0~R8) 파이프라인이다. **단일 진실 원천은 `refactor/` 폴더의 설계서 4종**이며, 본 문서의 이전 모드들(디자인 리뉴얼 v2, Sprint 6~10)과 충돌하는 내용은 refactor/ 문서가 우선한다.

### 우선순위 선언 (v2 조항 갱신)

- v2의 "CharacterFaceNode·NurseAvatarNode 본체 보호" 조항은 **해제** — R4에서 픽셀 포트레이트로 대체 후 삭제된다.
- `PixelSprite.swift`·`PixelPalette.swift`의 기존 16×20 데이터 byte-equal 원칙은 **계속 유효** (신규 데이터 추가만 허용).

### 자동 실행 절차

**0. 상태 확인 (필수 첫 단계)**: `REFACTOR_STATE.md`를 읽어 다음 Phase 결정.
- 직전 Phase "✅ 합격" → 다음 Phase / "❌ 불합격" → 같은 Phase 재시도(최대 3회) / 사용자가 R[N] 명시 → 강제 실행
- R0 → R1 → R2 → R3 → R4 → R5 → R6 → R7 → R8 순차.

**1. 하네스 호출 (기본 하네스 §단계 0~4 그대로)**

단계 0: `rm -f SPEC.md SELF_CHECK.md QA_REPORT.md`

단계 1: Planner 프롬프트에 다음을 추가:
```
refactor/00_MASTER_PLAN.md를 읽어라 (비전·불변 조건·합격 게이트).
현재 Phase의 주 설계서를 읽어라:
...
현재 Phase: R[N]
범위: 00_MASTER_PLAN §3 로드맵의 R[N] 행 + 주 설계서의 해당 섹션 그대로
```

단계 2~3: Generator → Evaluator (기본 하네스 동일. SELF_CHECK에 정량 게이트 측정치 필수)

단계 4: 판정 — 5축 채점 **패턴 20% / 기능 30% / 성능 20% / 시각 20% / 안정성 10%**, 가중 7.5 이상 합격.

**2. 상태 갱신**: REFACTOR_STATE.md의 해당 Phase 행 갱신(상태·점수·시도·완료일) + 진행 로그 한 줄 추가. 커밋 메시지 `[R{N}] 요약`.
...
- 변경 내용: [한 줄 요약]
- 수정 [n]개 / 신규 [m]개 / 삭제 [k]개 파일
- 정량 게이트: [측정치 — 노드 수, grep 0건 확인 등]
- QA 반복: X회 / 최종 점수: 패턴 X.X·기능 X.X·성능 X.X·시각 X.X·안정성 X.X (가중 X.X/10)
- 다음 Phase: R{N+1} ({이름}) 또는 "🎉 그랜드 리팩토링 전체 완료"
```

### 참고 문서 (그랜드 리팩토링 모드 전용)

| 파일 | 역할 |
|---|---|
| `refactor/00_MASTER_PLAN.md` | 비전·로드맵·합격 기준·불변 조건 (최상위) |
| `refactor/01_CODE_ARCHITECTURE.md` | 타겟 구조·GameConfig 분할·삭제 목록 |
| `refactor/02_GAME_FEEL.md` | 히트스톱·파티클·사운드·햅틱·메타 시스템 수치 |
| `refactor/03_UI_DESIGN_SYSTEM.md` | 디자인 시스템 v3·컴포넌트·화면 사양 |
| `refactor/AGENT_PROMPT.md` | 외부 코드 에이전트용 실행 프롬프트 사본 |
| `REFACTOR_STATE.md` | Phase 진행 상태 (하네스가 자동 갱신) |
