...
| **10-A** | 픽셀 시각 노출 (PlayerNode SVG 제거 → PixelSprite) | ✅ 합격 | 8.90/10 | 1/3 |
| **10-B** | 맵 크기·카메라 정합 (32×20 타일) | ✅ 합격 | 8.73/10 | 1/3 |
| **10-C** | 맵 빌더 3종 (벽) | ✅ 합격 | 9.05/10 | 1/3 |
| **10-D** | 수간호사 패트롤 + F 투척 | ✅ 합격 | 9.33/10 | 1/3 |
| **10-E** | 음표·F·A·변기·청진기 픽셀 + 스폰 | ✅ 합격 | 9.42/10 | 1/3 |
| **10-F** | 이교수·석조무사 픽셀 + AI | ✅ 합격 | 9.08/10 | 1/3 |
| **10-G** | 박병장 이스터에그 3단계 | ✅ 합격 | 9.08/10 | 1/3 |
| **10-H** | 컷씬 5종 | ✅ 합격 | 9.20/10 | 1/3 |
| **10-I** | 난이도·점수·콤보 수치 정합 | ✅ 합격 | 9.62/10 | 1/3 |
| **10-J** | HUD·iOS 이펙트 픽셀 톤 변환 | ✅ 합격 | 9.28/10 | 1/3 |
...
## Sprint 10 — 진행 로그 (원본 웹게임 1:1 픽셀 이식)

- 시작 예정: 2026-05-21
- 작업지시서: `SPRINT_10_REQUEST.md`
- 단일 진실 원천: `docs/ORIGINAL_GAME_ANALYSIS.md` (1391줄) + `docs/IOS_CURRENT_STATE.md`
- 사용자 의사결정: SPRINT_10_REQUEST.md §3 (7건 사전 확정 — 2026-05-21)
- Phase 10개 (A~J), 권장 순서 A → B → C/D → E → F → G → H → I → J
- 트리거: `Sprint 10 진행해줘` 또는 `Sprint 10 Phase X 진행해줘`

### Sprint 10.6 — 카운트다운 비가시 진단 + 폰트 회귀 (2026-05-21)

Sprint 10.5에서 분리된 진단 sprint. Sprint 10 Phase J에서 변경된 fontPixel("Menlo-Bold")이 큰 fontSize(120/140pt)에서 SKLabelNode 렌더링 회귀를 유발하는 것으로 진단 확정.

**진단 절차**:
1. CountdownNode 코드 + GameScene.showCountdown 호출부 정독 — 콘솔 로그 정상(onTick 3/2/1/onGo/onComplete 모두 발화), zPos 300 / parent=true / hidden=false / alpha=1.0 모두 정상
2. git log -- CountdownNode.swift — Sprint 7 Phase E(QA 9.76 합격) → Sprint 10 Phase J 두 commit 사이 단일 가시성 영향 변경 발견
3. git diff f4c3d93..c100cd5 — CountdownContext enum + init(context:) 신규에서 .ingame 분기 fontPixel 적용이 유일한 fontName 변경
4. 다른 fontPixel 사용처(HUD 18pt / ComboPopup 32pt / ScorePopup 28pt 등) 정상 렌더 → fontPixel 자체는 SKLabelNode 호환. **큰 fontSize(120/140pt)에서만 회귀** → SKLabelNode + Menlo-Bold + 큰 폰트 메트릭스 처리 실패 추정 (iOS 26.4 시뮬레이터)

**원인 확정**: Sprint 10 Phase J의 fontPixel 적용으로 카운트다운(fontSize 120pt/140pt)에서만 SKLabelNode 렌더링 회귀. 콘솔 로그 정상 출력 = SKAction.sequence는 정상 진행, 시각만 미렌더링.

**수정**: CountdownNode.init(context:) `fontName` 분기를 `GameConfig.fontDisplay` 단일 값으로 회귀 (Sprint 7 Phase E 시점 복원). 색 분기(.ingame 픽셀 화이트/옐로, .menu navy/coral)는 Phase J 그대로 유지 — *폰트만 회귀, 톤은 보존*.

**수정 파일**: 1개 — CountdownNode.swift line 49 1줄(fontName 직접 fontDisplay 사용) + enum 주석 + init 주석 보강.

**빌드 SUCCEEDED**: iPhone 17 시뮬레이터, 신규 워닝/에러 0.

**시각 변화**: 카운트다운 폰트가 Sprint 7 Phase E 톤(Jua-Regular)으로 복원. 색은 픽셀 톤 유지(3·2·1 픽셀 화이트, GO! 픽셀 옐로). HUD/ComboPopup 등 다른 fontPixel 사용처는 git diff 0줄 — 픽셀 톤 일관성은 큰 폰트(카운트다운) 1곳만 예외.

**잔존 P2**: 카운트다운 폰트가 픽셀 톤(Menlo-Bold)으로 100% 통일되지 않음. 후속 sprint에서 SKLabelNode + 픽셀 폰트(큰 fontSize 호환) 후보 시도 가능 ("Courier-Bold", "Menlo-Regular", "AmericanTypewriter-Bold"). 본 sprint는 *기능 회복*이 우선.

### Sprint 10.5 — 잔존 결함 6종 해소 (2026-05-21)

Sprint 10 직후 실기 시뮬레이터 검증에서 사용자가 발견한 6종 체감 결함 통합 해소. 카운트다운 비가시(이슈 3)는 진단 부족으로 별도 Sprint 10.6으로 분리.

**Phase A — UI 호흡 + 발사량 (이슈 1, 2, 5, 7)**
- `characterCardCenterYV9` 0.55 → 0.62 (다음 버튼 viewport 안으로) — 이슈 1
- `skillExplanationStatChipRowOffsetY` -60 → -78 (stat 칩과 힌트 24pt 호흡) — 이슈 2
- `projectileBurstCountByDifficulty` × 3 (1/3/4 → 3/9/12). 부채꼴 ±82.5° 그대로 OK — 이슈 5
- 신규 `resultScoreSubOffsetYV4 = -50` + ResultScene L592 V3→V4 (score↔sub 56pt 호흡) — 이슈 7

**Phase B — 음표 시각/PhysicsBody 분리 (이슈 4)**
- `noteSize` 16 → 32 (2x 정수 스케일, 캐릭터 32×40 대비 80% 크기)
- NoteNode.init에서 visualSize=32×32 / hitboxSize=16×16 분리 → 게임 밸런스 회귀 0

**Phase C — 수간호사 waypoint 셀 중심 정렬 (이슈 6a)**
- `nurseChiefWaypointsByDifficulty` (120,120)/(1120,120)/(1120,640)/(120,640) → (140,140)/(1140,140)/(1140,660)/(140,660)
- 원본 leftX=60 × SCALE 2 + 셀 중심 정합. 적 body 16×20이 row 3/16 셀 완전 수용, col 9/22 도어 통과 보장
- (120,120)은 셀 *모서리* 좌표라 body 하단 10pt 클립으로 벽에 끼는 원인 — Sprint 10.5에서 정정

**이슈 6b — F 벽 충돌 거동 유지**: ContactRouter 변경 0. 이슈 5 발사량 3배로 체감 보강.

**Sprint 10.6 분리 — 카운트다운 비가시 진단 (이슈 3)**: 폰트 Menlo-Bold는 iOS 정식 폰트 확인됨(다른 노드 정상 렌더). cameraNode position/scale, zPos 충돌, SKAction 키 중복 등 4가지 후보 후속 진단.

**변경 파일**: 3개 — GameConfig.swift(+7/-5: noteSize/burst dict/skillExpl chip/cardCenter/resultSub V4/waypoints dict), NoteNode.swift(+3/-1: visualSize/hitbox 분리), ResultScene.swift(+1/-1: V3→V4 1줄).

**변경 금지 git diff 0줄**: 메뉴 6 씬(CharacterSelect/SkillExpl/Result는 *값만* 변경, 본문 로직 0) + 메뉴 노드 14, PixelSprite/Palette/Renderer 본체, Phase A~J 모든 산물 본문, ContactRouter, SergeantPark/Airplane/BombFlash, SkillSystem/ScoreSystem 본문, PhysicsCategory, ColorTokens.

**빌드 SUCCEEDED**: iPhone 17 시뮬레이터, Phase A/B/C 각 검증 통과, 신규 워닝/에러 0.

### 🎉 Sprint 10 전체 완료 (Phase A~J, 평균 9.18/10, 2026-05-21)

10개 Phase 평균:
- Phase A 픽셀 시각 노출 — 8.90
- Phase B 맵 32×20 + 카메라 클램프 — 8.73
- Phase C 맵 빌더 3종 + WallTileNode — 9.05
- Phase D 수간호사 패트롤 + F/A — 9.33
- Phase E 음표/변기/청진기 픽셀 + 콤보 3단 — 9.42
- Phase F 이교수 8자 + 석조무사 4지점 + 자식 시각 제거 — 9.08
- Phase G 박병장 이스터에그 3단계 — 9.08
- Phase H 컷씬 5종 — 9.20
- Phase I 수치 봉인 (× SCALE + 콤보/스킬) — 9.62
- Phase J HUD/이펙트 픽셀 톤 변환 — 9.28

**평균 9.18/10** — 합격선 7.5 대비 +1.68 마진. 모든 Phase 1회차 통과 (재시도 0).

**원본 웹게임 1:1 픽셀 이식 완료**: 메뉴 v2 카툰 톤 보존 + 인게임 8-bit 픽셀 톤 통일 + 32×20 맵 byte-equal + 4 빌런 AI + 5명 스킬 + 콤보 3단 + 박병장 이스터에그 + 5종 컷씬.

### Sprint 10 Phase J — HUD·이펙트 픽셀 톤 변환 (✅ 합격 9.28/10, 1회차, 2026-05-21)

- **변경 내용**: 인게임 HUD/오버레이/이펙트의 모든 시각 톤을 v2 카툰(Jua-Regular + 코랄/네이비) → 8-bit 픽셀 톤(Menlo-Bold + 픽셀 옐로/페이퍼화이트) 일관 변환. 메뉴 v2 카툰 톤은 그대로 보존하여 시각 분기(메뉴=카툰 / 인게임=픽셀) 완성.
- **수정 파일**: 13개 — ColorTokens(픽셀 8색 추가), GameConfig(fontPixel + sparklePixelSize + tensionVignette 9 상수), HUDNode/HUDSkillSlotNode/ComboPopupNode/ComboBreakNode/ScorePopupNode(픽셀 폰트+색 swap), SparkleEffectNode(SparkleContext enum + init(context:) 분기 .ingame 픽셀/.menu 원형), CountdownNode(CountdownContext enum + 호환성 override convenience init), HitFlashNode(ganhoPixelHitRed + blendMode .add), GameScene(sparkle .ingame 2곳 + tensionVignette attach/detach 5줄), ResultScene(sparkle .menu 1줄), project.pbxproj(TensionVignetteNode 4 entries).
- **신규 파일**: TensionVignetteNode.swift(101 LOC, 4변 inset 비네트 + 깜빡임 0.3↔0.7 0.5s + zPos 110).
- **픽셀 톤 8색**: #FFD23F(HUD 옐로) / #FFFCE0(HUD 화이트) / #FF6E5A(코랄 경고) / #FFC830(콤보 골드) / #E0463A(콤보 레드) / #0F1118(외곽선 블랙) / #C8281A(HitFlash) / #FF3D2E(비네트).
- **context 분기 정책**: SparkleEffectNode/CountdownNode 둘 다 SparkleContext/CountdownContext enum 도입. 기본값 `.ingame` (호환성). 메뉴 사용처는 `.menu` 명시 — ResultScene SparkleEffect 호출 1줄만 메뉴 보호 위해 변경. GameScene SparkleEffect 2곳 `.ingame` 명시.
- **변경 금지 git diff 0줄**: PixelSprite/PixelPalette/PixelSpriteRenderer 본체, GameScene+Setup(게임 루프/스폰), ContactRouter, SergeantParkNode, AirplaneNode, BombFlashNode(Phase G 완료), 5종 컷씬 노드(Intro/Mid/IntroVillain/CutsceneOverlay/CutsceneTexts), 메뉴 6 씬+노드 14(ResultScene 1줄만 SPEC §18 예외), Phase A~I 산물.
- **금지 패턴**: 강제 언래핑 0 / Timer 0 / switch default 0 / as! 0 / update()-내-addChild 0 / SKAction 본문(시간/거리/scale) 0.
- **빌드 SUCCEEDED**: iPhone 17 시뮬레이터, 신규 워닝/에러 0.
- **5축**: 패턴 9.5 / 1:1 9.0 / 성능 9.0 / 시각 9.5 / 기능 9.5 (가중 9.28/10).
- **잔존 P2**: (1) HitFlash peakAlpha 0.6+.add 시뮬레이터 미세 조정 가능 (현 0.6 유지). (2) Portrait 회전 대응 — 가로 전용 게임이라 영향 0.

### Sprint 10 Phase I — 난이도·점수·콤보 수치 정합 (✅ 합격 9.62/10, 1회차, 2026-05-21)

- **변경 내용**: 원본 game.js §6.1 난이도 표 + 목표점수 + 5명 스킬 수치 byte-equal 봉인. playerSpeedStart/EndByDifficulty × SCALE=2 보정(140/160 → 280/320), stunDurationByDifficulty 신규(0.4/0.7/0.7), noteSpawnIntervalByDifficulty 신규(1.5/0.4/0.3). CharacterID.playerSpeedMultiplier 5명 모두 1.0 통일 (원본 L317). SpawnSystem.noteSpawnInterval 인스턴스 프로퍼티화.
- **수정 파일**: 3개 — GameConfig.swift(playerSpeedStart/End 갱신 + stun/noteSpawn dict 신규 + enemySpeed deprecated 주석), CharacterID.swift(playerSpeedMultiplier switch 9줄 → return 1.0 단일), SpawnSystem.swift(인스턴스 프로퍼티 + apply dict 적용 + startNoteSpawnLoop self 참조 + dead 프로퍼티 4종 주석).
- **원본 1:1 byte-equal**: §6.1 12행(baseSpeed/maxSpeed/notes/noteTtl/obstacles/obsBase/obsMax/stun/spawnInterval/maxObstacles/throwBurst) + L95 target(60/50/30) + L183~L197 SKILLS(jung 22/0.26/60, geon 20/0/120, im ∞/1.5/once, lee 22/0.5) + L1048~L1052 콤보 분기(≥3+2/≥5+3/≥7+4). 좌표·속도는 × 2, 시간/개수 그대로.
- **5명 스킬 수치 변경 0**: dashClimbDistance 60/Duration 0.26/Cooldown 22, bookClubRallyRadius 120/Cooldown 20, charmStudentDuration 1.5/oncePerGame true, taiwanTripInvulnerableDuration 0.5/Cooldown 22 — 모두 원본 1:1 grep 검증, SkillSystem 본문 0줄.
- **호출 순서 회귀 검증**: GameScene L322 spawnSystem.apply → L324 spawnSystem.start 순서로 startNoteSpawnLoop 시점에 self.noteSpawnInterval dict 값 갱신됨. apply 누락 시도 default 1.5 fallback.
- **변경 금지 git diff 0줄**: 메뉴 6 씬+노드 14, PixelSprite/PixelPalette/PixelSpriteRenderer 본체, Phase A~H 산물, HUD/이펙트, ContactRouter, 5종 컷씬 노드, ScoreSystem, SkillSystem, GameScene+Setup.
- **금지 패턴**: 강제 언래핑 0 / Timer 0 / switch default 0 / as! 0 / update()-내-addChild 0 / 매직 넘버 0.
- **빌드 SUCCEEDED**: iPhone 17 시뮬레이터, 신규 워닝/에러 0.
- **5축**: 패턴 9.5 / 1:1 9.8 / 성능 9.5 / 시각 10.0 / 기능 9.0 (가중 9.62/10).
- **회귀 우려 5건 모두 안전**: (1) playerSpeed ×2 — 빌런도 Phase D ×2 적용, 상대 비율 보존. (2) playerSpeedMultiplier 통일 — 라벨 6곳만 변경, 로직 분기 0. (3) noteSpawn 차등 — 동시 상한+TTL 자가 조정. (4) SpawnSystem 인스턴스화 — apply→start 순서+fallback 이중. (5) enemySpeed deprecated — 호출처 0 grep.
- **잔존 P2 5건 (모두 후속 sprint)**: stunDurationByDifficulty 적용 사이트(위험 2 의도적 보류), enemySpeedStart/End 삭제(OQ-B), SpawnSystem dead 프로퍼티 4종 삭제(OQ-B), taiwanTripJumpDistance 알고리즘 정합(OQ-A), 노트 frame-fill 정밀화(OQ-C).

### Sprint 10 Phase H — 컷씬 5종 (✅ 합격 9.20/10, 1회차, 2026-05-21)

- **변경 내용**: 원본 5종 컷씬(intro 250ms 캐릭터별 / mid1 timeLeft ≤30 / mid2 ≤15 / introStoneGuard easy+normal / introProfessor hard)을 iOS에 1:1 이식. 신규 정적 팩토리 3종(IntroCutsceneNode/MidCutsceneNode/IntroVillainCutsceneNode) + CutsceneTexts 단일 진실 원천. UserDefaults intro 영구 스킵 제거 → 매 판 1회 발화(cutscenesShown Set, restart 시 removeAll).
- **신규 파일**: 4개 — CutsceneTexts.swift(5종 본문 lookup), IntroCutsceneNode.swift(250ms 지연 정적 팩토리), MidCutsceneNode.swift(mid1/mid2 정적 팩토리), IntroVillainCutsceneNode.swift(난이도 분기 정적 팩토리).
- **수정 파일**: 4개 — GameConfig(신규 상수 4: cutsceneIntroDelay=0.25, mid1Threshold=30, mid2Threshold=15, pixelCutsceneFontName="Menlo-Bold"), GameScene(cutsceneShown Set + UserDefaults 분기 제거 + 컷씬 메서드 위임 + update mid1/mid2 폴링), CutsceneOverlayNode(fontName 옵셔널 +8줄, 본체 로직 0줄), project.pbxproj(4 파일 × 4 entries = 16줄).
- **원본 1:1 임계**: intro 250ms (game.js L2268 byte-equal), mid1 ≤30s, mid2 ≤15s, introStoneGuard "마주치면 잡혀갑니다" easy+normal, introProfessor "청진기에 맞으면 정지" hard, 매 판 1회 발화 (원본 state.cutscenesShown Set 일치).
- **캐릭터별 본문 placeholder**: 원본 game.js L200~L233 캐릭터별 한국어 본문 로컬 부재로 placeholder TODO 명시. introStoneGuard/introProfessor는 기존 iOS 정합 텍스트(GameConfig.stoneGuardWarning*/professorWarning*) 그대로 재사용 — 일치도 보장.
- **CutsceneOverlayNode 최소 변경**: present에 fontName: String? = nil 매개변수 1개 추가, configure*Label 3개에 `if let fontName { label.fontName = fontName }` 1줄씩 — 총 6줄 본체 + 2줄 시그니처 = 8줄 diff. 본체 로직(터치/fadeIn/fadeOut/dismiss/nil 토글/sequence) 0줄.
- **트리거 흐름**: didMove → IntroCutsceneNode.present(250ms 지연) → onDismiss 난이도 분기 → IntroVillainCutsceneNode.present → 그 dismiss 후 .countdown + showCountdown. update 매 프레임 mid1/mid2 임계 체크 + Set.contains O(1) 가드 + .cutscene 진입 시 update 본체 자동 정지(Phase 7-3 .cutscene 가드 재활용).
- **변경 금지 git diff 0줄**: PixelSprite/PixelPalette/PixelSpriteRenderer, SergeantParkNode, 메뉴 6 씬+노드 14, ContactRouter, HUD/HUDSkillSlot/5명 SkillSystem, MusicNoteEmitter/NoteNode/EnemyNode/StoneGuardNode/ProfessorNode 인게임/AirplaneNode/BombFlashNode/DiplomaOverlayNode.
- **금지 패턴**: 강제 언래핑 0 / Timer 0 / switch default 0 / as! 0 / update()-내-addChild 0 / 매직 넘버 0.
- **빌드 SUCCEEDED**: iPhone 17 시뮬레이터, 신규 워닝/에러 0.
- **5축**: 패턴 9.5 / 1:1 8.5 / 성능 9.5 / 시각 9.5 / 기능 9.5 (가중 9.20/10).
- **잔존 P1**: CutsceneTexts intro/mid1 캐릭터별 본문 placeholder — 원본 game.js 접근 가능 시 단일 진입점 갱신으로 해결.
- **잔존 P2**: (1) update mid1/mid2 polling 미세 최적화. (2) cutscenesShown.insert 시점 이동 검토.

### Sprint 10 Phase G — 박병장 이스터에그 3단계 픽셀 톤 통합 (✅ 합격 9.08/10, 1회차, 2026-05-21)

- **변경 내용**: 석조무사 접촉(easy/normal) 시 발동되는 박병장 이스터에그를 iOS 컷씬 보존 + 원본 3단계 시퀀스 결합으로 정교화. t=0.0 오버레이+SergeantParkNode 클로즈업+EnemyNode.startFleeing 동시 발화 → t=2.4 비행기 가로지름 → t=3.4 폭탄 섬광+purgeAllF → t=5.0 flee 종료+F 재시딩. AirplaneNode/BombFlashNode/AirforceOverlayNode 픽셀 톤 통일.
- **수정 파일**: 8개 — AirplaneNode(6 자식 SKShape → 16×5 도트 픽셀 SKTexture, crossScreen 시그니처 보존), BombFlashNode(blendMode .add + ganhoPixelFlashWhite + peakAlpha 0.92), AirforceOverlayNode(Menlo-Bold + ganhoPixelWarning), EnemyNode(startFleeing 본문 강화 + updatePatrol/updateThrowStateMachine isFleeing 가드 + throwTimer 복원), PixelSpriteRenderer(texture(rows:palette:scale:) helper 추가, 본체 byte-equal 보존), SpawnSystem(purgeAllF + currentObstaclesTarget getter 신설), GameScene(triggerAirforceEasterEgg 3단계 직렬화), GameConfig(신규 상수 9 + UIColor extension 4색).
- **원본 1:1 byte-equal**: 비행기 16×5 도트 game.js L3328~L3336, fleeSpeed 180 px/s L131, bombFlash 420ms (fadeIn 0.07 + fadeOut 0.35) L137, F 전멸 + A 보존 L3419, fleeDuration 5000ms L132, 단위벡터 (chief.x>=halfMap ? 1 : -1)/√2 L3454~L3470.
- **iOS 컷씬 보존**: SergeantParkNode.makeIntroCloseup() 재사용, zPosition 210 (overlay 200 < closeup 210 < bombFlash 250), fadeIn 0.1 + stay 1.6 + fadeOut 0.5 = 2.2s → 비행기 등장 t=2.4 자연 연결.
- **purgeAllF**: enumerateChildNodes(withName: "projectile") + ProjectileNode 타입체크 + isEnchanted=false 가드. A(name "aItem")는 자연 보존.
- **F 재시딩**: flee onEnd 콜백에서 spawnSystem.currentObstaclesTarget vs 현재 F 개수 deficit 만큼 fireImmediately 호출 (원본 L2678 Math.round(obstacles × 1.0) 일치).
- **isFleeing 가드**: updatePatrol/updateThrowStateMachine 진입부 1줄씩 — 도주 중 patrol 정지 + 텔레그래프/투척 무력화.
- **변경 금지 git diff 0줄**: 메뉴 6 씬+노드 14, SergeantParkNode 본체(호출만), ContactRouter(Phase G 단독), PixelSprite/PixelPalette/PixelSpriteRenderer 본체(helper extension만), ColorTokens 본체(GameConfig inline UIColor extension로 우회), PhysicsCategory, Phase A~F 산물, HUD/이펙트/스킬(Phase J), 5명 SkillSystem(Phase I).
- **금지 패턴**: 강제 언래핑 0 / Timer 0 / switch default 0 / as! 0 / update()-내-addChild 0.
- **빌드 SUCCEEDED**: iPhone 17 시뮬레이터, 워닝 0.
- **5축**: 패턴 9.5 / 1:1 9.0 / 성능 9.0 / 시각 8.5 / 기능 9.5 (가중 9.075/10).
- **잔존 P2**: (1) SergeantParkNode 카툰 톤 vs 비행기 픽셀 톤 시각 분리 — 사용자 #2 의도. (2) PressStart2P 미설치 시 Menlo-Bold 폴백 — 픽셀 폰트는 Phase J 통합. (3) blendMode .add 폭탄 섬광 — 0.42s 1회 부하, 60fps 안전.

### Sprint 10 Phase F — 이교수·석조무사 픽셀 + AI (✅ 합격 9.08/10, 1회차, 2026-05-21)

- **변경 내용**: ProfessorNode 8자 패트롤(좌표 (120,100)/(520,280)/(520,100)/(120,280), 70 px/s) + 청진기 텔레그래프 0.4s 코럴핑크 ! 깜빡임 + 시작 오프셋 12px + 첫 투척 3.0s. StoneGuardNode 4지점 순환(좌표 (80,80)/(540,80)/(540,300)/(80,300), 55 px/s) + 픽셀 텍스처 부착. EnemyNode/ProfessorNode/StoneGuardNode 자식 시각(성적표/Hello PPT/클립/돌상 갑옷/일자눈/청진기 disc/tube) 전체 제거 (사용자 결정 #3). 청진기 명중 토스트 1s → freeze 2s 직렬화.
- **수정 파일**: 9개 — EnemyNode(자식 5메서드 본문 삭제+color clear 제거), ProfessorNode(자식 제거+selectInitialWaypoint+startPatrolFrom+텔레그래프+fireStethoscope 12px+초기 3s), StoneGuardNode(픽셀 텍스처+selectInitialWaypoint+startPatrolFrom+updatePixelAnimation+자식 제거), PixelSprite(stoneGuardData extension), PixelPalette(stoneGuardPalette inline), GameConfig(좌표 표 2 재정합 + 신규 상수 6), GameScene+Setup(selectInitialWaypoint 호출 2줄), GameScene(onStethoscopeHitPlayer 토스트→freeze 직렬화), project.pbxproj(ProfessorTelegraphNode 4 entries).
- **신규 파일**: ProfessorTelegraphNode.swift (EnemyTelegraphNode 패턴, 색 .ganhoCoralPrimary).
- **원본 1:1 byte-equal**: 8자 좌표 (game.js L109~L115/L2983~L3019), 4지점 (L119~L122/L3221~L3274), 텔레그래프 0.4s (L583), 시작 오프셋 12px (L586), 청진기 220 px/s × 4발 (Phase E), 회전 0.628s (Phase E), 토스트 1s + freeze 2s (L4060~L4084), StoneGuard 16×20 매트릭스 + 7키 팔레트 (L3120~L3192).
- **자식 시각 제거**: setupVisualOverlay/attachHalo/attachChart/attachClip/applyVisualScaleV9 (EnemyNode), attachStethoscopeDisc/Tube/scaleV9 (ProfessorNode), attachArmor/attachEyes/scaleV9 (StoneGuardNode). 본체 color=.clear/colorBlendFactor=1.0도 제거 (픽셀 본체 노출).
- **난이도 매트릭스**: 이교수 hard만, 석조무사 easy+normal만 (기존 guard 일치 확인).
- **변경 금지 git diff 0줄**: 메뉴 6 씬+노드 14, StethoscopeNode 본문(Phase E 보호), PixelSpriteRenderer/PixelSprite/PixelPalette 본체(helper extension만), PhysicsCategory 비트, Phase A~E 산물, SergeantPark/Airplane/BombFlash(Phase G), HUD/이펙트/스킬(Phase J).
- **금지 패턴**: 강제 언래핑 0 / Timer 0 / switch default 0 / as! 0 / update()-내-addChild 0.
- **빌드 SUCCEEDED**: iPhone 17 시뮬레이터, 워닝/에러 0.
- **5축**: 패턴 9.0 / 1:1 9.5 / 성능 8.5 / 시각 9.0 / 기능 9.0 (가중 9.08/10).
- **잔존 P2**: (1) 텍스처 재생성 캐시 최적화 (refreshTexture 호출 빈도) — Phase J/I. (2) ProfessorNode.throwStethoscope sequence withKey 부여(endGame 차단 정합성) — Phase G. (3) 메뉴 카드용 Professor SVG path 잔존 여부 후속 검증.

### Sprint 10 Phase E — 음표·변기·청진기 픽셀 + 콤보 3단 (✅ 합격 9.42/10, 1회차, 2026-05-21)

- **변경 내용**: 음표(NoteNode) 12×12 fillRect 5단계 원본 좌표 byte-equal + 글로우/펄스 자식 제거 + bob 애니메이션(±2.4px, 0.7s, phase 랜덤). 변기(ToiletNode) 16×20 매트릭스 + 4색 #ffffff/#cfd3da/#a9d6ef/#1a1a22 원본 drawToilet byte-equal. 청진기(StethoscopeNode) 14×8 매트릭스 + 3색 + 회전 0.628s 정확. 콤보 가산 이진 분기 → 3단 분기(≥3 +2 / ≥5 +3 / ≥7 +4). SpawnSystem dead code 5메서드 제거.
- **수정 파일**: 8개 — NoteNode(자식 3→0, bob), ToiletNode(본문 0줄, Pixel 자동 반영), StethoscopeNode(size 28×16, texture 교체), PixelSpriteRenderer(notePixelTexture/stethoscopeTexture/variableMatrixTexture helper 신규), PixelSprite(toiletData extension), PixelPalette(toiletPalette extension), SpawnSystem(dead code 5메서드 -65 LOC), ScoreSystem(recordNoteHit 3단 분기), GameConfig(8 신규 상수 + 0.628 정정).
- **원본 1:1 byte-equal**: 음표 fillRect 5단계+하이라이트 (game.js L730~L785), 변기 7단계 매트릭스 (L756~L869), 청진기 14×8 (L2922~L2960), 콤보 분기 (L811~L817/L1048~L1052), 회전 2π×0.1=0.6283185s.
- **bob 애니메이션**: ±2.4px y 이동(원본 1.2×SCALE 2), 0.7s 주기, 인스턴스마다 phase 랜덤 (동시 스폰 동조 방지), withKey "noteBob" 멱등.
- **변기 ×2**: recordToiletBonus → recordNoteHit 2회 호출 → 새 3단 분기 자동 적용 (원본 §4.7.4 정확 일치).
- **dead code 정리**: startProjectileFireLoop / scheduleNextFire / currentFireInterval / fireProjectile / currentProjectileCount 5메서드 제거 (Phase D OQ-6 해소).
- **변경 금지 git diff 0줄**: MusicNoteEmitterNode(메뉴), PhysicsCategory/ColorTokens, ToiletNode.init/applyLifetime 본문, ContactRouter/GameScene+Setup, PixelSpriteRenderer/PixelSprite/PixelPalette 본체, ScoreSystem 다른 메서드, SpawnSystem 음표/변기 스폰, Phase A·B·C·D 산물, 메뉴 6 씬+노드 14.
- **금지 패턴**: 강제 언래핑/Timer/switch default/as!/update()-내-addChild 0건.
- **빌드 SUCCEEDED**: iPhone 17 시뮬레이터.
- **5축**: 패턴 9.5 / 1:1 9.7 / 성능 9.2 / 시각 9.0 / 기능 9.5 (가중 9.42/10).
- **잔존 P2**: (1) SpawnSystem dead 프로퍼티 4개 — Phase I 정리. (2) stethoscopeSize(18) 상수 미사용 — Phase I deprecated. (3) ToiletNode 16×20→16×16 vertical squish 0.8배 — padding 영역이라 시각 영향 미미, Phase J 검토.

### Sprint 10 Phase D — 수간호사 패트롤 + F 투척 + A 매혹 분기 (✅ 합격 9.33/10, 1회차, 2026-05-21)

- **변경 내용**: 수간호사(EnemyNode) 직선 추적 폐기 → 4지점 사각 순환 패트롤(easy 2점 왕복 / normal Z패턴 4점 / hard 시계방향 4모서리). 빨강 ! 텔레그래프 0.4초 깜빡임(120ms 주기) 후 ±15° 스프레드 burst 발사. 임간호 매혹 활성 시 F→A 변신(점수 ×2 수집 아이템).
- **수정 파일**: 12개 — EnemyNode.swift(추적 폐기, 패트롤+텔레그래프 상태 머신+burst fireF), PixelSpriteRenderer(12×12 가변 helper), PlayerNode(contactTest aItem 1줄), PhysicsCategory(aItem=256 1줄), GameConfig(약 80줄 상수), SpawnSystem(fire loop 폐기, fireImmediately 위임), ContactRouter(aItem 분기 33줄), GameScene(L510 enemy.update 시그니처 + aItem 콜백), GameScene+Setup(setupEnemy provider 주입), project.pbxproj(3 신규 12 entries).
- **신규 파일**: 3개 — EnemyTelegraphNode.swift(빨강 ! 깜빡임), FProjectileNode.swift(12×12 F 픽셀 #ff3b4e), AItemNode.swift(12×12 A 픽셀 #ff6fa8).
- **원본 1:1 byte-equal**: 패트롤 좌표 (60,60)/(560,60)/(560,320)/(60,320) × 2 = (120,120)/(1120,120)/(1120,640)/(120,640). 패트롤 속도 40/60/100 px/s × 2 = 80/120/200 pt/s. F obs 60-110/170-290/200-340 → iOS 120-220/340-580/400-680. 스프레드 π/12 ± 0.025 jitter. 텔레그래프 0.4s + 120ms 깜빡임 + #ff3b4e. startOffset 24pt (원본 12px × 2). burst 1/3/4. hypot 최댓값 시작 위치 (game.js L2618~L2628).
- **F/A 분기**: 발사 시점 1회 검사 (원본 game.js L489~L495 byte-equal). 매혹 만료 후 이미 발사된 A는 그대로 A 유지(별도 노드).
- **PhysicsCategory.aItem 신규 비트**: 256 (0b100000000). PlayerNode contactTestBitMask 1줄 추가, 다른 영역 0.
- **SpawnSystem 호환**: fireImmediately = enemy.fireFOnce() 위임 — 박병장 이스터에그(Phase G) 회귀 0.
- **변경 금지 git diff 0줄**: NurseAvatarNode(메뉴), SkillSystem(charmStudent 자연 noop), ProjectileNode(dead code), 메뉴 6 씬 + 노드 14, PixelSprite/PixelPalette, Models/Repos/Managers, HUD/이펙트.
- **금지 패턴 0건**: 강제 언래핑 / Timer / switch default / as! / update()-내-addChild / didBegin 직접 removeFromParent.
- **빌드 SUCCEEDED**: iPhone 17 시뮬레이터.
- **5축**: 패턴 9.5 / 1:1 9.5 / 성능 9.0 / 시각 9.0 / 기능 9.5 (가중 9.33/10).
- **잔존 P2**: (1) SpawnSystem dead code 5개 메서드(132~199) 호출 0건 — Phase E 일괄 정리 권장. (2) 12×12 매트릭스 텍스처 미캐싱 — 인스턴스마다 UIGraphicsImageRenderer 재호출, 캐시 도입 권장.

### Sprint 10 Phase C — 맵 빌더 3종 (벽) (✅ 합격 9.05/10, 1회차, 2026-05-21)

- **변경 내용**: 원본 game.js L262~L313 buildMap의 easy/hard 두 맵 데이터를 byte-equal로 iOS 이식. WallTileNode 신규 (40×40pt SKSpriteNode + 정적 physicsBody). MapNode.buildWalls(difficulty:) 본문 작성 — buildOuterWall + buildEasyInterior / buildHardInterior 분기. normal·hard 동일 빌더 공유(원본 DIFFICULTY 매핑 byte-equal).
- **수정 파일**: 5개 — WallTileNode.swift(신규 53 LOC), MapNode.swift(buildWalls 본문 + 7 헬퍼, 64→202 LOC), GameConfig.swift(Wall Tile 섹션 + Easy 4 + Hard 4 방 × 7 + 중앙 기둥 4 × N 상수, 옛 hardMap* 33 + normalMap* 14 상수 삭제), GameScene+Setup.swift(setupMap 1줄 위임 + 옛 빌더 7 함수 + 외곽 라운드 보더 삭제, 548→308 LOC), project.pbxproj(WallTileNode 4 entries).
- **원본 1:1 byte-equal**: 좌상/우상/좌하/우하 4 방 가로벽·세로벽·문 좌표(좌상 m[3][9]=0, 우상 m[3][22]=0, 좌하 m[16][9]=0, 우하 m[16][22]=0) + 중앙 기둥 4(m[9..10][12]/[19], m[7]/[12][15..16]). 좌표 변환 `iosRow = 19 - origR` 13개 호출 지점 일관.
- **PhysicsCategory.wall 비트 0줄 변경**: wall 비트(0b1000=8) 이미 존재, Player collision / Enemy collision / Projectile contactTest / Stethoscope contactTest 모두 정상 연결 — 사용자 8 노드 본문 0줄.
- **외곽 라운드 보더 삭제**: addOuterWalls 본문 삭제와 함께 사라짐(OQ-1). 원본 1:1 우선, Phase J 픽셀 외곽 효과 재도입 검토.
- **옛 상수 호출자 0**: hardMap* 33 + normalMap* 14 상수 삭제 안전성 grep 검증(주석 도큐멘트만 잔존). breakableWallName 보존(SkillSystem.dashClimb 사용자).
- **변경 금지 ~80개 파일 git diff 0줄**: Phase A·B 산물 + 메뉴 6 씬 + 메뉴 노드 14 + 픽셀 엔진 3 + 충돌 마스크 8 노드 + PhysicsCategory + Systems/Managers/Repositories/Models 본문.
- **금지 패턴**: 강제 언래핑 0 / Timer 0 / DispatchQueue 0 / as! 0 / switch default 0 / update() 본문 내 addChild 0.
- **빌드 SUCCEEDED**: iPhone 17 시뮬레이터, iOS 26.5 SDK.
- **5축**: 패턴 9.0 / 1:1 9.5 / 성능 8.5 / 시각 8.5 / 기능 9.5 (가중 9.05/10).
- **벽 노드 수**: Easy 104셀(외곽 96+중앙 8), Hard 140셀(외곽 96+4방 36+중앙 8) — SpriteKit 공간 분할 60fps 안전.
- **잔존 P2 (합격 영향 0)**: (1) outerWallBorder* dead code 정의 잔존 — Phase J 정리 후보. (2) mapColumns/mapRows/tileSize 호환 별칭 — 후속 Phase 정리. (3) buildHardInterior LOC 60줄 권장 리팩토링. (4) Phase D enemy 시작점 vs 우상 방 가로벽 20pt 차(OQ-6) — Phase D 4지점 패트롤 첫 점 정의에서 해소.

### Sprint 10 Phase B — 맵 크기·카메라 정합 (✅ 합격 8.73/10, 1회차, 2026-05-21)

- **변경 내용**: iOS 좌표 그릇을 원본 game.js와 1:1 정합. mapColumns 48→32, mapRows 24→20, tileSize 20pt→40pt(CELL_PT 승격). worldWidth 960→1280pt, worldHeight 480→800pt. MapNode 신규(SKNode + tileCoordinate/worldSize/buildWalls 후크). 카메라 follow에 맵 가장자리 클램프 (`updateCameraFollow()`).
- **수정 파일**: 5개 — GameConfig.swift(원본 1:1 8 상수 + tileSize/mapColumns/mapRows 산식 갱신), MapNode.swift(신규), GameScene.swift(mapNode 1줄 + Camera Follow 섹션 + L501 1줄 치환), GameScene+Setup.swift(setupWorld worldNode.addChild(mapNode) 1줄), project.pbxproj(MapNode 4 entries).
- **원본 1:1**: TILE(20)/COLS(32)/ROWS(20)/SCALE(2)/CELL(40)/W(1280)/H(800) 모두 ORIGINAL_GAME_ANALYSIS.md L14~L24 §0 + L101 SCALE 라인 명시. game.js L18/L19/L17/L101/L713.
- **카메라 클램프**: `if upperX < lowerX → worldW/2 중앙 고정` 분기로 viewport > world 안전 처리. lower=halfW/halfH, upper=worldW-halfW/worldH-halfH, max/min 산술 O(1).
- **자동 적응 5건**: player 초기 (240,240)→(320,400), enemy 초기 (720,360)→(960,600), 외곽 벽 mapW/mapH 자동 확장, 체크보드 1152→640 노드(메모리 -44%), setupCamera 초기 (480,240)→(640,400).
- **Phase A 회귀 0**: PlayerNode.swift Phase B 세션에서 Edit 호출 0건. pixelSpriteChild 32×40pt가 새 셀(40pt)에 정확 정합 (가로 32≤40, 세로 40=40).
- **변경 금지 ~80개 파일 git diff 0줄**: PlayerNode + 메뉴 6씬 + 메뉴 노드 14 + 픽셀 엔진 3 + Color/Physics 2 + Domain/Models + Repositories + Managers + Systems + 빌런/투사체 9 + HUD/이펙트 15.
- **금지 패턴 grep**: Phase B 4파일에서 강제 언래핑 0 / Timer 0 / DispatchQueue 0 / as! 0 / switch default 0 / update() 본문 내 addChild 0 / 매직 넘버 1280/800/40/32/20 직접 하드코딩 0건. tileSize 호출자 35건 모두 산식 의존 자동 정합.
- **빌드 SUCCEEDED**: xcodebuild iPhone 17 시뮬레이터, Swift 컴파일 워닝/에러 0건.
- **5축 채점**: 패턴 9.0 / 1:1 9.0 / 성능 8.5 / 시각 8.5 / 기능 8.5 (가중 8.725/10).
- **잔존 P1 (Phase C 책임 영역, Phase B 합격 영향 0)**: GameConfig L541~588 hardMap 좌표(c=38/43, r=21 등) + L500~520 normalMap 좌표가 새 좌표계 32×20 범위 초과 → 외곽 벽 일부 맵 밖 배치 가능. easy 난이도는 회귀 0(addCentralPillar만 호출). Phase C에서 WallTileNode 신규 + MapNode.buildWalls 본문 채우기 + 원본 game.js L289~309 좌표 byte-equal 재추출 책임.
- **잔존 P2**: enemy 초기 위치 (960,600)이 hard 맵 우상 방과 겹칠 가능성 → Phase C/I 미세조정.

### Sprint 10 Phase A — 픽셀 시각 노출 (✅ 합격 8.90/10, 1회차, 2026-05-21)

- **변경 내용**: PlayerNode가 자식으로 부착하던 CharacterFullBodyNode(SKShapeNode 풀바디 카툰) 제거 → PixelSpriteRenderer로 만든 16×20 픽셀 SKSpriteNode 자식 1개로 통일. 5명 캐릭터 × 4방향 = 20 SKTexture 정적 캐시. 메뉴 카드는 카툰(SVG path) 그대로 보존하여 시각 톤을 인게임(픽셀)과 메뉴(카툰)로 명확히 분리.
- **수정 파일**: 2개 — PlayerNode.swift (+129/-83, 362줄), CharacterFullBodyNode.swift (헤더 1줄만, 본문 0줄)
- **원본 1:1 매핑**: CharacterID rawValue가 원본 game.js ID(kim/jung/geon/im/lee)와 byte-equal 동형, Direction→PixelDirection 정확 매핑(.front→.down, .back→.up, .left/.right 동일). 원본 분석 라인 출처: ORIGINAL_GAME_ANALYSIS.md L137~L171 §2.3 + L175~L273 §2.5.
- **신규 API**: `attachPixelSpriteChild(for:)` instance, `cachedTexture(for:direction:)` static, `pixelDirection(from:)` static. 정적 캐시 `textureCache: [CharacterID: [PixelDirection: SKTexture]]` lazy 채움 (메모리 상한 25.6KB).
- **PNG 우선 경로 제거**: `loadTexture(for:direction:frame:)` static 메서드 완전 삭제 — Sprint 10은 PNG 미사용, PixelSpriteRenderer 직행.
- **본체 self.texture placeholder 차단**: `color=.clear` + `colorBlendFactor=1.0` (Sprint 9 Phase B 검증된 패턴) — 자식 1개가 시각 단일 진실 원천.
- **회귀 0**: physicsBody/categoryBitMask/velocity/update(deltaTime:)/freeze(duration:)/apply(Difficulty)/isInvulnerable/isFrozen 본문 0줄 변경.
- **변경 금지 19개 파일 git diff 0줄 확인**: PixelSprite/PixelPalette/PixelSpriteRenderer + CharacterFaceNode/CharacterCardNode/DifficultyCardNode/NurseAvatarNode/ProfessorNode(SVG path) + 6 메뉴 씬 + ColorTokens/PhysicsCategory + GameScene/GameScene+Setup.
- **금지 패턴 grep**: 강제 언래핑 0 / Timer 0 / DispatchQueue 0 / as! 0 / switch default 0 / update() 본문 내 addChild 0.
- **빌드 SUCCEEDED**: xcodebuild iPhone 17 시뮬레이터, Swift 컴파일 워닝/에러 0건.
- **5축 채점**: 패턴 9.0 / 1:1 9.0 / 성능 9.0 / 시각 8.5 / 기능 9.0 (가중 8.90/10).
- **잔존 P2 (Phase B 합격 영향 0)**: (1) PlayerNode L97 본체 self.texture가 .kim down placeholder 보유 — color/blend 차단으로 시각 영향 0이나 1.28KB 추가 보유. (2) PlayerNode L305 매직 넘버 0.1(velocity 임계값) GameConfig 상수화 후보. (3) CharacterFullBodyNode.swift 파일 자체 외부 참조 0 — 후속 Phase에서 삭제 후보(SPRINT_10_REQUEST.md §2.2). (4) tickWalkFrame이 자식 texture 미반영(idle 고정) — SPEC §8.10 의도적 정책, Phase 후속에서 walk 부활.
